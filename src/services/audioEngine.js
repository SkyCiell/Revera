...
export function playSynthSample(sample, iemCurve = null) {
  ...
  osc2.type = 'sine';
  osc2.frequency.setValueAtTime(freq * 0.5, audioCtx.currentTime); // Sub-harmonic

  // Create Parametric EQ Biquad Filter Chain to simulate IEM sound profile
  filterNodes = [];
  ...
  // Apply IEM frequency curve deltas if provided
  ...
  // Connect chain: oscs -> low -> mid -> pinna -> treble -> masterGain -> analyser -> destination
